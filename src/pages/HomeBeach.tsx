...
export default function HomeBeach() {
  const { t } = useTranslation();
  const { t: tHome } = useTranslation('homeBeach');
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 56 }}>
      <Head
        title={tSeo('homeBeach.title')}
        description={tSeo('homeBeach.description')}
        path="/beach"
        jsonLd={[
          buildWebSite(lang),
          buildBreadcrumb(
      ...
      <section style={{ paddingTop: 24 }}>
        ...
        <p style={{ fontSize: 17, maxWidth: 520, margin: '0 0 28px 0', color: 'var(--ink)', opacity: 0.75 }}>
          {tHome('hero.subtitle')}
        </p>
